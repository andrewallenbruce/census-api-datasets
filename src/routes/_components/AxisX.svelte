...
<script>
  ...
  const { width, height, xScale, yRange, containerWidth } = getContext('LayerCake');
  /**
  ...
  //choose the tick marks to label based on width
  let ticks = $derived(
    $containerWidth > 500 ? ticks_wide : ticks_narrow
  );
